...
class DataLoader:
    ...
    def load_all_data(dates=None, base_dir=None):
        ...
        for date_str in target_dates:
            # Look for normalized CSV for this date
            normalized_file = os.path.join(normalized_dir, f"normalized_mapping_{date_str}.csv")
            
            if not os.path.exists(normalized_file):
                print(f"⚠️ Skipping {date_str}: normalized CSV not found")
                continue
            ...
            try:
                ...
                if 'stock' in df.columns:
                    ...
                    df['Stock'] = 'Yes'
                df['Promotion Details'] = ''  # No promo data in normalized CSV
                df['Payment Promo'] = ''  # No payment promo data
