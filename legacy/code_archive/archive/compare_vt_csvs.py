...
def compare_csvs(file1, file2):
    ...
    # Viettel names are usually clean, but let's strip
    ...
    if df1['Key'].duplicated().any():
        print(f"Warning: Duplicates in Old CSV: {df1[df1['Key'].duplicated()]['Key'].tolist()}")
    if df2['Key'].duplicated().any():
        print(f"Warning: Duplicates in New CSV: {df2[df2['Key'].duplicated()]['Key'].tolist()}")
    ...
    if only_in_1:
        print(f"\n[MISSING IN NEW] ({len(only_in_1)}) items:")
        for k in list(only_in_1)[:10]:
            print(f"  - {k}")
        if len(only_in_1) > 10: print("  ... and more")

    if only_in_2:
        print(f"\n[NEW IN NEW] ({len(only_in_2)}) items:")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python compare_vt_csvs.py <old_csv> <new_csv>")
    else:
