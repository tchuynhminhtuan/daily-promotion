...
class Apple:
    def check(self):
        df = pd.read_csv(f"../content/{now}/Daily Promo Pricing - {now_1}.csv", delimiter=";")
    ...
    def create_Apple_dashboard_2(self):
        # 1 start fpt
        fpt_df = pd.read_csv(f"../content/{now}/1-fpt-{now}.csv", delimiter=";")
        ...
        new_fpt=pd.DataFrame(data, columns = fpt_column_names)
        fpt_df=pd.concat([new_fpt, fpt_df], ignore_index = True)
        ...
        mw_df = pd.read_csv(f"../content/{now}/2-mw-{now}.csv", delimiter=";", low_memory=False)
        mw_not_trade_list=[
        ...
        new_mw = pd.DataFrame(data, columns = mw_column_names)
        mw_df = pd.concat([new_mw, mw_df], ignore_index=True)
        ...
        mw_replacement_dict = {
            ...
            'MacBook Air 13 inch M3 8GB/256GB (MRXT3SA/A)': 'Macbook Air M3 13 2024 8CPU 8GPU/8GB/256GB',
        ...
        # 3 start hh
        hh_df = pd.read_csv(f"../content/{now}/3-hh-{now}.csv", delimiter=";")
        hh_not_trade_list = [
        ...
        hh_new = pd.DataFrame(data, columns = hh_column_names)
        hh_df = pd.concat([hh_new, hh_df], ignore_index = True)
        ...
        hh_replacement_dict = {
            ...
            'MacBook Pro 14 inch M3 8GB/512GB': 'MacBook Pro 14 2023 M3 8 CPU/10 GPU/8GB/512GB',
            ...
            'Apple Watch Ultra 2 2024 GPS + LTE, 49mm Vỏ Titan Dây Đeo Ocean': 'Apple Watch Ultra 2 49mm viền Titanium Dây Ocean Band',
            'MacBook Air M2 15 8GB/256GB': 'MacBook Air 15 inch M2 2023 8CPU 10GPU 8GB/256GB',
            'MacBook Air M2 15.3 inch 8GB/256GB': 'MacBook Air 15 inch M2 2023 8CPU 10GPU 8GB/256GB',
            ...
            'MacBook Air M2 13.6 inch 8GB/256GB': 'MacBook Air M2 2022 13 inch 8CPU 8GPU 8GB 256GB',
            ...
            'MacBook Air M3 15.3 inch 8GB/256GB': 'Macbook Air M3 15 2024 8CPU 10GPU/8GB/256GB',
            'MacBook Air M3 15 8GB/512GB': 'Macbook Air M3 15 2024 8CPU 10GPU/8GB/512GB',
            'MacBook Air M3 15.3 inch 8GB/512GB': 'Macbook Air M3 15 2024 8CPU 10GPU/8GB/512GB',
            'MacBook Air M3 13 8GB/256GB': 'Macbook Air M3 13 2024 8CPU 8GPU/8GB/256GB',
            'MacBook Air M3 13.6 inch 8GB/256GB': 'Macbook Air M3 13 2024 8CPU 8GPU/8GB/256GB',
            'MacBook Air M3 13 8GB/512GB': 'Macbook Air M3 13 2024 8CPU 10GPU/8GB/512GB',
            'MacBook Air M3 13.6 inch 8GB/512GB': 'Macbook Air M3 13 2024 8CPU 10GPU/8GB/512GB',
        ...
        def to_str(val):
            return str(val)
        ddv_df = pd.read_csv(f"../content/{now}/4-ddv-{now}.csv", delimiter=";", converters={"Gia_Khuyen_Mai": to_str})
        ...
        ddv_new = pd.DataFrame(data, columns = ddv_column_names)
        ddv_df = pd.concat([ddv_new, ddv_df], ignore_index = True)
        ...
        # 5 start vt
        vt_df = pd.read_csv(f"../content/{now}/5-vt-{now}.csv", delimiter=";")
        vt_not_trade_list=[
            'iPhone 12 64GB',
        ...
        vt_new = pd.DataFrame(data, columns = vt_column_names)
        vt_df = pd.concat([vt_new, vt_df], ignore_index = True)
        ...
        # 6 start cps
        cps_df = pd.read_csv(f"../content/{now}/6-cps-{now}.csv", delimiter=";")
        ...
        new_cps = pd.DataFrame(data, columns = cps_column_names)
        cps_df = pd.concat([new_cps, cps_df], ignore_index = True)
        ...
        # 7 start shopee
       #  shopee_df = pd.read_csv(f"../content/{now}/7-shopee-{now}.csv", delimiter=";")
       ...
       #
       #  shopee_new = pd.DataFrame(data, columns = shopee_column_names)
       #  shopee_df = pd.concat([shopee_new, shopee_df], ignore_index = True)
       #
       ...
       #
        ...
        # 8 start tz
        tz_df = pd.read_csv(f"../content/{now}/8-topzone-{now}.csv", delimiter=";")
        ...
        new_tz = pd.DataFrame(data, columns = tz_column_names)
        new_tz['vnpay']=0
        tz_df = pd.concat([new_tz, tz_df], ignore_index = True)
        ...
        # # 9 start tiki
        # tiki_df = pd.read_csv(f"../content/{now}/9-tiki-{now}.csv", delimiter=";")
        #
        # tiki_not_trade_list= \
        ...
        #
        # new_tiki=pd.DataFrame(data, columns = tiki_column_names)
        # tiki_df=pd.concat([new_tiki, tiki_df], ignore_index = True)
        #
        ...
        # 10 start sd
        sd_df = pd.read_csv(f"../content/{now}/10-sd-{now}.csv", delimiter=";", on_bad_lines='skip')
        ...
        sd_new = pd.DataFrame(data, columns = sd_column_names)
        sd_df = pd.concat([sd_new, sd_df], ignore_index = True)
                                    ...
                                    try:
                                        ...
                                        try:
                                            ...
                                            try:
                                                ...
                                                try:
                                                    max_vnpay_sd=n[(n.index("giảm") + 5):(n.index("đ"))].replace(
                                                        ".",
                                                        "").replace(",",
                                                                    "")
                                                    print(max_vnpay_sd, "max_vnpay_sd")
                                                            ...
                                                            for sd_item in sd_products:
                                                                if fpt_item in sd_item:
                                                                    ...
                                                                    km_CPS=cps_df[
                                                                        ...
                                                                        0]
                                                                    # Check if km_CPS is a string, and apply replace methods only if it is
                                                                    if isinstance(km_CPS, str):
                                                                        km_CPS=km_CPS.replace("₫", "").replace(".",
                                                                                                               "").replace(
                                                                            "đ", "")
                                                                    else:
                                                                        # If it's a float, convert it to string or leave it as a float
                                                                        km_CPS=str(km_CPS)
        ...
        df=pd.DataFrame(data = data_list,
                            columns = ["Product Name", "Promo Price @ FPT",
                                       ...
                                       "Stock @ FPT", "Stock @ MW", "Stock @ TopZone",
                                       ...
                                       "Link @ SD", "Link @ HH", "Link @ DDV", "Link @ CPS", "Date"])

        df.insert(loc = 1, column = "Lowest Price", value = int)
        test = []
        for i, r in df.iterrows():
            list_check = []
            for ii in r[2:16].to_list():
                try:
                    ii = int(ii)
        ...
        new_df['Stock Status'] = pd.cut(new_df['Stock Status'], bins = [0, 4, 6, 9], labels = choices,
                                            right = True)
