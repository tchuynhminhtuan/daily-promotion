...
class DDVScraper(BaseScraper):
    ...
    async def scrape_tech_specs(self, page):
        """Scrape tech specs using user-provided selectors."""
        try:
            # 1. Expand content if button exists
            expand_btn = page.locator("//button[@class='more-content flex items-center justify-center absolute z-50 bottom-0 left-0']")
            ...
            # 2. Extract text from container
            container = page.locator("//div[@class='flex-col relative section-layout !p-4']")
