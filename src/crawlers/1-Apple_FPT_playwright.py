...
class FPTScraper(BaseScraper):
    ...
    def get_fieldnames(self):
        fields = [
            ...
            "Date", "Khuyen_Mai", "Thanh_Toan", "Other_promotion", "Link"
        ]
    ...
    async def scrape_product_data(self, page, url, forced_color=None, do_screenshot=True):
        ...
        if os.environ.get("SCRAPE_SPECS") == "True":
            try:
                ...
                print(f"Error scraping tech specs: {e}")
    ...
    async def scrape_tech_specs(self, page):
        """Clicks 'Thông số kỹ thuật' and scrapes the drawer content."""
        try:
            # 1. Find and Click
            specs_btn = page.locator("//span[@class='cursor-pointer text-textOnWhiteHyperLink f1-medium pc:b2-medium']").first
            if await specs_btn.count() > 0:
                await specs_btn.click(force=True)
                
                # 2. Wait for Drawer
                drawer = page.locator("div[class*='Swipeable_swipeable'][class*='Swipeable_right']")
                try:
                    ...
                    # 4. cleanup: Close drawer
                    # Try clicking the X button first
                    close_btn = page.locator(".sheet-title svg").first
