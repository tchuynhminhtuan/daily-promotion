...
class ViettelScraper(BaseScraper):
    ...
    async def scrape_tech_specs(self, page):
        """Clicks 'Xem thêm' and scrapes tech specs."""
        try:
            # 1. Expand content
            expand_btn = page.locator("//div[@id='hanghoa']//div[@id='navcau-hinh']")
            if await expand_btn.count() > 0 and await expand_btn.is_visible():
                try:
                    await expand_btn.click(force=True, timeout=2000)
                    await page.wait_for_timeout(1000) # Wait for animation/modal
                except: pass
            
            # 2. Extract text from container
            container = page.locator("//div[@id='hanghoa']")
